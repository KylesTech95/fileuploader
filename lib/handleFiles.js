...
import { handleFileSize, filesize } from './handleFileSize.js';
...
export function fileSystemChange(e){
    ...
    if((files.length > 0 && (files.length+document.querySelectorAll('.file-obj-entity').length) <= maxFiles)){
        ...
        scrolltab.style.left = !filesize + "px" ? document.querySelector('.file-icon-img').width+"px" : filesize + "px"
    } else {
...
function enableScroll(target,filesize){
    !filesize ? null : scrolltab.style.left = filesize + "px"
